...
// UL: according to https://bugs.launchpad.net/linaro-android/+bug/1081597 test for mHwc->blank
status_t HWComposer::release(int disp) {
    ALOGI("HWComposer::release on display %d called", disp);
...
status_t HWComposer::acquire(int disp) {
    ALOGI("HWComposer::acquire on display %d called", disp);
    LOG_FATAL_IF(disp >= HWC_NUM_DISPLAY_TYPES);
    if (mHwc && mHwc->blank) {
